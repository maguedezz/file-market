    ...
    @slot('links')
        <div class="level">
            <div class="level-left">
                ...
                <form action="{{ route('admin.files.new.update',$file->identifier) }}" id="approve-{{ $file->id }}" method="POST" class="is-hidden">
                    @csrf
                    @method('PATCH')
                </form>
                <p class="level-item">
                ...
                <form action="{{ route('admin.files.new.destroy',$file->identifier) }}" id="reject-{{ $file->id }}" method="POST" class="is-hidden">
                    @csrf
                    @method('DELETE')
                </form>
            </div>
